...
The key is a \emph{selection function} (see~\cite[Th.~6.37]{blackburn2001modal}), whose novelty here is the ability of handling data comparisons. A selection function enables us, given a model and a formula, to retain only the parts of the model that are relevant for evaluating the formula. Remarkably, the resulting model has size polynomial in that of the original model and the formula, thereby yielding a \emph{small-model property}.  
In what follows, we will use~$*$ to refer indistinctly to $=$ or $\neq$ in data comparisons.
...
\begin{definition}\label{def:xp:tbox}
    A \emph{terminological box} (\emph{TBox}) is a set $\tbox$ of expressions of the form $p \doteq \psi$, where $p$ is a concept name, and $\psi$ is a concept.
    ...
    \end{enumerate}
    For every acyclic $\tbox$ and every ${p \doteq \varphi} \in \tbox$, we say that $\varphi$ defines $p$ and call $\varphi$ the definition of $p$.
\end{definition}
...
\begin{definition}\label{def:xp:abox}
    An \emph{assertion box} (\emph{ABox}) is a set $\abox$ of expressions of the form:
    \[ 
...
\begin{definition}\label{def:xp-kb-semantics}
   ...
   % We call $\tbox$ \emph{consistent} iff there is $\model$ s.t.\ $\model \vDash \tbox$.
    ...
    We say that an ABox $\abox$ is \emph{consistent} iff there exists $\model,\naming$ such that  $\model,\naming \vDash \abox$.
...
It is not hard to see how $\sel$ can be extended to handle nominals. We simply need to add cases $\sel(i{:}\varphi, n) = \sel(\varphi,\naming(i))$ and $\sel((i,j){:}\alpha, n) = \sel(\alpha,\naming(i))$. With this new definition at hand, we can tackle the problem of deciding whether an ABox is consistent. Then, the next result follows by adapting the argument in the proof of \Cref{lemma:selection}.
...
Namely, $\varphi$ is satisfiable w.r.t.\ $\tbox$ iff $p$ is satisfiable w.r.t.\ $\tbox \cup \{p \doteq \varphi\}$, for $p$ a new concept name, i.e., $p$ not in $\tbox$ and $\varphi$.
...
If no such a $p \doteq \varphi$ is in $\tbox$, we can just add $\tbox \cup \{p \doteq q \lor \lnot q\}$, for $q$ a new concept name. This modification of $\tbox$ does not affect satisfiability.
...
\begin{proof}
    ...
    \begin{description}
        ...
        Replace all $p\doteq\psi\in\tbox$ for $p\doteq\nnf(\psi)$.
        Then, add $p'\doteq\nnf(\neg\psi)$ for $p'\in\props$ new, and replace occurrences of $\lnot{p}$ with $p'$ in $\tbox$.
        Note that $\neg\tup{\alpha=_\cmp\beta}$ already is in NNF.
    ...
    \end{description}
    The rules 1 to 3 above terminate in polynomial time and create a simple TBox $\tbox'$ which is polynomial on the size of $\tbox$.
...
\begin{definition}
    ...
    Let $\model = \tup{ \N, \D, \set{\R_{\apath}}_{\apath \in \mods}, \valuation,\set{\data_\cmp}_{\cmp \in \cmps} }$ be a data model, $n\in\N$, and $\tbox$ be a simple TBox.
    Define the \emph{selection function} $\sel$ as:
    \[
    \begin{array}{@{\ }l@{\,}l@{}@{\ }l}
        \sel(\apath,n) & = \begin{cases}
...
The first distinction between the definition of the selection function in \Cref{def:selection:tbox} and that in~\Cref{def:selection} is the case of propositional symbols. In \Cref{def:selection:tbox}, if a propositional symbol $p$ is such that $p\doteq\psi \in \tbox$, we need to proceed in unfolding such a definition recursively. Even though a TBox provides global information about the model, we only unfold the definitions that are relevant from the evaluation point, thus, we only take into  account definitions in the TBox locally. The second distinction is a minor one, and concerns the definition of a case of disjunction. We need this case now, as in simple TBoxes we deal with formulas in negation normal form. 
...
%    To simplify notation let $\model' = \model{\restriction}_{\sel(p,n)}$.
    Item 1 is immediate. From the way in which $\V'$ is defined, we know that for each concept name $q$ s.t.\ ${q \doteq \psi} \in \tbox$, we have ${\truthset{q}{\model{\restriction}_{\sel(p,n)}} = \truthset{\psi}{\model{\restriction}_{\sel(p,n)}}}$.
...
The problem of concept satisfiability w.r.t. an acyclic TBox in $\xpath$ is \np-complete.
\end{theorem}