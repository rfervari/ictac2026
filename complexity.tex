...
We pay attention to the complexity of \emph{concept satisfiability},  ABox \emph{consistency} and \emph{concept satisfiability w.r.t. acyclic TBoxes} for $\xpath$ expressions. In particular, we will show that $\xpath$ can be actually seen as a fragment of $alcd$ enjoying nice computational properties.
...
In a nutshell, given a model, $\sel$ selects the relevant pieces on which to evaluate a given node expression. The next lemma states that this selection preserves the truth value of a node expression.

\begin{lemma}\label{lemma:selection}
    Let $\varphi$ be a node expression.
    Moreover, let ${\model = \tup{\N, \D, \set{\R_{\apath}}_{\apath \in \mods}, \valuation,\set{\data_\cmp}_{\cmp \in \cmps}}}$ be s.t.\ ${\truthset{\varphi}{\model}\neq\emptyset}$.
    Then, for all $\{n,m\}\subseteq\N$, all path expressions $\alpha$, and all node expressions~$\varphi$, the following statements are true:
    \begin{enumerate}[1.]
        \item $(n,m)\in\truthset{\alpha}{\model}$ iff $(n,m)\in\truthset{\alpha}{\model{\restriction}_{\sel(\alpha,n)}}$;
        \item $n\in\truthset{\varphi}{\model}$ iff $n\in\truthset{\psi}{\model{\restriction}_{\sel(\varphi,n)}}$. %\model{\restriction}_{\sel(\varphi,n)}
    \end{enumerate} 
    Moreover, $\size{\sel(\varphi,n)} \leq \size{\varphi} + 1$.
\end{lemma}
...
\begin{proof}
    ...
    \item[Base cases.] \
        \begin{description}
            \item[($\alpha = \apath$).]
            Left-to-right: let $(n,m)\in\truthset{\apath}{\model}$, i.e., $(n,m)\in\R_\apath$.
            We know $\sel(\apath,n)=\set{n,\R_\apath(m)} = \set{n,m}$.
            Then, $(n,m)\in (\R_\apath \cap ((\sel(\apath,n))^2))$.
            So, $(n,m)\in\truthset{\apath}{\model{\restriction}_{\sel(\apath,n)}}$. The right-to-left direction is similar.
            
            \item[($\varphi = p$).]
            Left-to-right: let $n\in\truthset{p}{\model}$, i.e., $n\in\V(p)$.
            Then, $n\in(\V(p) \cap \sel(p,n))$.
    ...
    \item[Inductive cases.] \
        \begin{description}
            ...
            We know $(n,\R_\apath(n))\in\R_\apath$ and $(\R_\apath(n),m)\in\truthset{\alpha}{\model}$.
            ...
            From the def.~of $\sel$, it follows that $\{n,m,m'\}\subseteq\sel(\tup{\alpha=_{\cmp}\beta},n)$.
    ...
    It remains to show that  $\size{\sel(\varphi,n)} \leq \size{\varphi} + 1$, where $\size{\varphi}$ is the number of modal symbols of $\varphi$. The proof is by induction on $\varphi$.
