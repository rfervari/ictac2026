...
In this section we introduce DL with concrete domains.
More precisely, we present the logic $\alcd$~\cite{BaaderH91}.
This logic can be seen as the logic $\alc$ extended with:
  \emph{abstract features},
  \emph{concrete features}, and, more importantly, a
  \emph{concrete domain} $\cdom$ (which can be seen as a parameter of the logic).
...
\begin{definition}[Concrete Domain]\label{def:conc:domain}
    A \emph{concrete domain} is a pair $\cdom = \tup{\Delta_\cdom, \Phi_\cdom}$ where:
      ...
      $\Phi_\cdom$ is a set of $n$-ary predicate names s.t.\
        each $P \in \Phi_\cdom$ has an extension $P^\cdom \subseteq \Delta_\cdom^n$.
    A concrete domain is \emph{admissible} if:
      (i) $\Phi_\cdom$ it is closed under negation, i.e.,
        for each $P\in\Phi_\cdom$ there is $\bar{P}\in\Phi_\cdom$ such that $\bar{P}^\cdom=\Delta_\cdom\setminus P^\cdom$,
      (ii) there exists $\toppred \in \Phi_\cdom$ s.t.\
        $\toppred^\cdom=\Delta_\cdom$, and
      (iii) there exists an algorithm for deciding satisfiability of finite conjunctions of the form
        % $\bigwedge_{P_i\in\Phi_\cdom} P_i(\mathbf{x}_i)$ (with $\mathbf{x}_i$ a tuple of variables of the appropriate arity)
        $\bigwedge\setof{P_i(\mathbf{x}_i)}{ i \in [1 \dots k]}$, where $P_i \in \Phi_{\cdom}$ and $\mathbf{x}_i$ is a sequence of variables whosef length is the arity of $P_i$.
\end{definition}

Below we assume that $\cdom$ is an arbitrary but fixed concrete domain. Moreover, we assume that 
$N_C=\set{C, D, C_1, C_2 , \dots}$ is a countable set of \emph{concept names},
$N_R= \set{r, r_1, r_2 , \dots, r, \dots}$ is a countable set of \emph{role names},
$N_{\mathit{aF}}=\set{f, f_1, f_2 , \dots} \subseteq N_R$ is a countable set of \emph{abstract features}, and
$N_{\mathit{cF}}=\set{g, g_1, g_2 , \dots}$ is a countable set of \emph{concrete features}. 
...
\begin{definition}[Language] \label{def:alcd-syntax}
  ...
  The well-formed expressions of $\alcd$, called \emph{concepts}, are defined by the grammar:
  \[
    C, D \Coloneqq
    C_i \mid
    \lnot C \mid
...
Models of this logic are called \emph{interpretations}. We will show later on the correspondence between data models and interpretations. 

\begin{definition}[Interpretation]\label{def:interpretation}
    An \emph{interpretation} is a structure defined as
    ${\interp = 
    ...
    \begin{enumerate}
      ...
      \item 
        ...
        \begin{enumerate}
          ...
          \item 
            every  $f_i \in N_{aF}$, to a functional binary relation $\sem{r_i}$ on $\Delta_{\interp}$;
          \item 
            every $g_i \in N_{cF}$, to a partial function $g^\interp : \Delta_\interp \nrightarrow \Delta_D$.
          % \item 
  ...
  \begin{definition} [Semantics] \label{def:alcd:semantics} 
    Let ${\interp = \tup{\Delta_{\interp}, {\cdot^{\interp}}}}$ be an interpretation.
    The semantics of $\alcd$ concepts is defined as: 
    \begin{align*}
      (\lnot C)^\interp                  ={}& \Delta^\interp \setminus C^\interp \\
      (C \sqcap D )^\interp              ={}&  {C^\interp} \cap {D^\interp} \\
      (\exists R.C )^\interp             ={}& \setof{x}{ \text{exists } y \text{ s.t } (x, y) \in R^\interp \text{ and } y \in C^\interp } \\
      (\exists u_1g_1 \dots u_ng_n.P)^\interp  ={}&  \{x \mid \text{exists } {y_i \in \dom(g_i^\interp)} \text{ s.t } (x,y_i) \in u_i^\interp \\
      & \quad\quad \text{ and } (g_1^\interp(y_1) \dots g_n^\interp(y_n)) \in P^\cdom \}.
  \end{align*}
  \end{definition}
