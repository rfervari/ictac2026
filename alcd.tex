...
In this section we present the logic $\alcd$~\cite{BaaderH91}.
This logic is the logic $\alc$ extended with:
  \emph{abstract} and \emph{concrete features}, and, more importantly, a
  \emph{concrete domain} $\cdom$.
We introduce the syntax and semantics of this logic below.

Let
  $N_C=\set{C, D, C_1, C_2 , \dots}$ be a countable set of \emph{concept names},
  $N_R= \set{r, r_1, r_2 , \dots, r, \dots}$ be a countable set of \emph{role names},
  $N_{\mathit{aF}}=\set{f, f_1, f_2 , \dots} \subseteq N_R$ be a countable set of \emph{abstract features}, and
  $N_{\mathit{cF}}=\set{g, g_1, g_2 , \dots}$ be a countable set of \emph{concrete features}. 
...
The semantics of $\alcd$ is given in terms of \emph{interpretations}. Later on we will relate data models and interpretations as alternative presentations of the same kind of structure.

\begin{definition}[Concrete Domain]\label{def:conc:domain}
    \val{donde usamos esta definicion?}
...
In what follows, we assume that $\cdom$ is an arbitrary but fixed concrete domain.
...
\begin{definition}[Interpretation]\label{def:interpretation}
  ...
  \begin{definition} [Semantics] \label{def:alcd:semantics} 
    ...
    \begin{align*}
      ...
      (\exists R.C )^\interp             ={}& \setof{x}{ \text{exists } y \text{ such that } (x, y) \in R^\interp \text{ and } y \in C^\interp } \\
      (\exists u_1g_1 \dots u_ng_n.P)^\interp  ={}&  \{x \mid \text{exists } {y_i \in \dom(g_i^\interp)} \text{ such that } \\
      & \qquad (x,y_i) \in u_i^\interp \text{ and } (g_1^\interp(y_1) \dots g_n^\interp(y_n)) \in P^\cdom \}.
  \end{align*}
  \end{definition}
